...
> **Companion to** `docs/ux/information-architecture.md` (Increment 1 base) and lo-fi `docs/ux/lo-fi/increment-2-click-and-collect.md` / `.drawio` (checkout and staff patterns extended — **no new lo-fi per plan waiver**). Specification-stage spec; implementation and tests land in Engineering (interface-design implementation pass → ATDD → clean code). Extends Increment 2 interface spec and prototype under `packages/`.
...
| Lo-fi reference | **Derived** from `docs/ux/lo-fi/increment-2-click-and-collect.md` + `docs/ux/information-architecture.md` (plan waiver — no `increment-3` lo-fi file) |
| Acceptance criteria | `docs/story/acceptance-criteria/increment-3-acceptance-criteria.md` |
| Specification by example | `docs/story/specification-by-example/increment-3-specification-by-example.md` |
| Domain terms | `docs/domain/ubiquitous-language.md` |
| Architecture reference | `docs/architecture/architecture-reference.md` (Order Placement extension, Unified Order Queue, Ship-to-Home Fulfillment, Shipping Notification, Order Status Page & Guest Lookup) |
| Prior interface spec | `docs/ux/increment-2-interface-design.md` |
...
Same baseline as Increment 2 (`docs/ux/increment-2-interface-design.md`); additions only where Increment 3 introduces new surfaces.
...
Affordances for Increment 2 screens not listed above: unchanged from `increment-2-interface-design.md` / lo-fi.
