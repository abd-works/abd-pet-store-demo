...
import { resolvePreferredPickupSelection } from '../../../store/client/preferredPickupSelection';
...
export function PickupStoreSelectionPage() {
  ...
  useEffect(() => {
    setSelectedCode((current) => resolvePreferredPickupSelection(preferredStoreCode, current));
  }, [preferredStoreCode]);
        ...
        <section aria-label="pickup store selection">
          ...
          <ul data-testid="pickup-store-list" style={{ listStyle: 'none', padding: 0, marginTop: 16 }}>
            {stores.map((store) => (
              <li
                key={store.storeCode}
                data-selected={selectedCode === store.storeCode ? 'true' : 'false'}
                style={{
                  border: selectedCode === store.storeCode ? '2px solid #111' : '1px solid #ddd',
                  padding: 12,
                  marginBottom: 8,
                }}
              >
