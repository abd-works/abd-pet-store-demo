...
| **Engineering** | Clickable prototype, object model, acceptance tests (ATDD), production code | *In* (prototype opt in when UI-heavy) |
| **Regression testing** | Broader regression suite maintenance | *In* |
| **User testing / UAT** | End-user validation outside the team | *Out* |

ATDD is **skill 3 inside Engineering**, not a separate stage. Thin slicing is **last in Discovery**, not a separate prioritization stage.
...
**Not default (opt in):** shaping, discovery, exploration extras (UX mockup, arch template), engineering clickable prototype, user testing / UAT.
