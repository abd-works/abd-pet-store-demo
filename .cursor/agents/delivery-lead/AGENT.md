...
**Authoritative progress (disk):** `<workspace>/docs/planning/delivery-war-room/` — orchestration and run/stage checkboxes in **`delivery-plan-checklist.md`** (generated by **`track_task`**); slot execution state in **`slot-NN-finished.md`** / **`run-log.jsonl`**. The narrative plan stays at **`docs/planning/abd-delivery-lead/agile-delivery-plan.md`**. On **Step 2** (and when resuming), read the plan and war room before building or revising. When the plan is **confirmed** or **revised** (including after **Step 7**), update the plan and **regenerate** the war-room checklist so they stay aligned.
...
- **`abd-delivery-war-room`** (`../skills/abd-delivery-war-room/SKILL.md`) — War room protocol, templates, and slot file conventions. **Read this before Step 2** when running with the CLI harness. Templates under `templates/` provide the file formats for `manifest.md`, `INSTRUCTIONS.md`, slot files, and `profile.md`.
...
- **`execute-skill-using-skills-rules`** (`skills/skill-helpers/execute-skill-using-skills-rules/SKILL.md`) — **Corrections.** When you identify wrong or missing deliverables, gate failures, or new constraints, log them in **`docs/corrections-log.md`** using the skill’s **correction process** (same contract as **`delivery-team-member`**): identify → log with DO / DO NOT and **Example (wrong)** → direct rework; do not substitute informal chat for a log entry when a fix must stick for downstream work.
- **`track_task`** (`skills/skill-helpers/track_task/SKILL.md`) — Mandatory. Follow the skill for workspace resolution, checkbox rules, and **each-turn** updates. Use the skill’s **war room checklist** section for where to write the file and what lines to include (orchestration + full plan: runs, stages, checkpoints).
...
**Team roles** (family → role): [../../content/roles/team-roles.md](../../content/roles/team-roles.md). Stages often assign **extension roles** (e.g. Business Expert for domain skills, UX Designer for IA) — not only the stage default.
...
A **stage** is not one team-member session. A stage is a **sequence of slots** — often **multiple team members**, **multiple family roles**, and **multiple practice skills** — until the stage **exit gate** in `stages/<stage>.md` passes.

| Unit | Meaning |
| --- | --- |
| **Stage** | Bootcamp phase (Shaping → … → Engineering): entry conditions, skill order, exit gate |
| **Slot** | One harness handoff: `slot-NN-start.md` → team member work → `slot-NN-finished.md` |
...
Tick **each** matching line in `delivery-plan-checklist.md` as the pair progresses (executor block, **reviewer scanned**, **reviewer reviewed**, rework, delivery-lead gate). Slot numbers increment across the whole engagement — executor and reviewer slots are **separate** slot IDs.
...
Plan slot sequences in `manifest.md`. Author **one slot start at a time** (harness pattern): finish slot NN, validate, then write slot NN+1 (executor or reviewer as appropriate).
...
- `skills/skill-helpers/track_task/SKILL.md`
...
- regenerate `docs/planning/delivery-war-room/delivery-plan-checklist.md` by running `python skills/skill-helpers/track_task/scripts/generate_delivery_checklist.py` (or equivalent per `track_task`)
...
- **Executor:** `team-role` matches the skill's family role in `stages/<stage>.md`; `skills:` lists the practice skill(s) for this slot; run scope is exact.
...
Instantiate an **executor** `delivery-team-member`. **When running with the CLI harness**, write `slot-NN-start.md` using `abd-delivery-war-room` `templates/slot-start.md`:

```yaml
team-role: <family role for this skill — product-owner | business-expert | ux-designer | engineer>
stage: <current stage>
skills:
...
**Without the harness:** bootstrap executor or reviewer directly in chat with the same payload fields; still write finished files to the war room when disk progress matters.
...
When **all skill pairs** in the stage are complete, verify the **stage** exit gate and cross-stage consistency. **Tick every matching line** in `delivery-plan-checklist.md`: each skill's executor block, **reviewer scanned**, **reviewer reviewed**, rework (or N/A), then **delivery lead — exit gate verified**. A stage is not done when one executor finishes — only when the full stage gate passes after all pairs.
...
- mark the stage block complete in `docs/planning/delivery-war-room/delivery-plan-checklist.md`
...
**When running with the CLI harness:** append slot/stage events to `run-log.jsonl`. Write `slot-(NN+1)-start.md` for the **first executor pair** of the next stage (or next skill in the same stage if you have not finished the stage — normally you finish all pairs before Step 6).
...
- **Track in the war room.** Follow **`track_task`**; tick executor, reviewer, and rework lines separately in **`delivery-plan-checklist.md`**.
...
You instantiate **multiple** `delivery-team-member` agents per stage — **executors** (family roles) and **reviewers** (`team-role: reviewer`) — each in its own slot. Their contract is in `delivery-team-member/AGENT.md`.
